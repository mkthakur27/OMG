...
//	public void save(int itemId,String size, int quantity, double price);
	...
	@Insert("insert into cart_item_map(cartid, itemid, size, quantity) values(#{cartId}, #{itemId}, #{size}, #{quantity})")
	public void addToCart(int cartId, int itemId, String size, int quantity);
	
	@Insert("insert into cart(isactive) values(#{isActive})")
	public void addNewCartId(short isActive);
